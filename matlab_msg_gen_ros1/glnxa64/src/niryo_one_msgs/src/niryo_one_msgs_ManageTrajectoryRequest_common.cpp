...
// Common copy functions for niryo_one_msgs/ManageTrajectoryRequest
...
namespace ManageTrajectoryRequest {
  void copy_from_arr_niryo_one_msgs_Trajectory(niryo_one_msgs::Trajectory& val, const matlab::data::StructArray& arr);
  MDArray_T get_arr_niryo_one_msgs_Trajectory(MDFactory_T& factory, const niryo_one_msgs::ManageTrajectoryRequest::_trajectory_type& val);
  ...
  //----------------------------------------------------------------------------
  NIRYO_ONE_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<niryo_one_msgs::ManageTrajectoryRequest>& msg, const matlab::data::StructArray arr) {
    try {
        //cmd_type
    ...
    try {
        //trajectory_id
        const matlab::data::TypedArray<int32_t> trajectory_id_arr = arr[0]["trajectory_id"];
        msg->trajectory_id = trajectory_id_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'trajectory_id' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'trajectory_id' is wrong type; expected a int32.");
    }
    try {
        //trajectory
  ...
  //----------------------------------------------------------------------------
  NIRYO_ONE_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const niryo_one_msgs::ManageTrajectoryRequest>& msg) {
    auto outArray = factory.createStructArray({1,1},{"cmd_type","trajectory_id","trajectory"});
    // cmd_type
    outArray[0]["cmd_type"] = factory.createScalar(msg->cmd_type);
    // trajectory_id
    outArray[0]["trajectory_id"] = factory.createScalar(msg->trajectory_id);
    // trajectory
