...
// Common copy functions for niryo_one_msgs/generatetrajResponse
...
namespace generatetrajResponse {
  //----------------------------------------------------------------------------
  NIRYO_ONE_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<niryo_one_msgs::generatetrajResponse>& msg, const matlab::data::StructArray arr) {
    try {
        //positions
  ...
  //----------------------------------------------------------------------------
  NIRYO_ONE_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const niryo_one_msgs::generatetrajResponse>& msg) {
    auto outArray = factory.createStructArray({1,1},{"positions","velocities","acceleration"});
    // positions
    outArray[0]["positions"] = factory.createArray<niryo_one_msgs::generatetrajResponse::_positions_type::const_iterator, double>({1, msg->positions.size()}, msg->positions.begin(), msg->positions.end());
    // velocities
    outArray[0]["velocities"] = factory.createArray<niryo_one_msgs::generatetrajResponse::_velocities_type::const_iterator, double>({1, msg->velocities.size()}, msg->velocities.begin(), msg->velocities.end());
    // acceleration
    outArray[0]["acceleration"] = factory.createArray<niryo_one_msgs::generatetrajResponse::_acceleration_type::const_iterator, double>({1, msg->acceleration.size()}, msg->acceleration.begin(), msg->acceleration.end());
