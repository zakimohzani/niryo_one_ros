...
namespace SetLedsRequest {
  //----------------------------------------------------------------------------
  NIRYO_ONE_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<niryo_one_msgs::SetLedsRequest>& msg, const matlab::data::StructArray arr) {
    try {
        //values
  ...
  //----------------------------------------------------------------------------
  NIRYO_ONE_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const niryo_one_msgs::SetLedsRequest>& msg) {
    auto outArray = factory.createStructArray({1,1},{"values"});
    // values
    outArray[0]["values"] = factory.createArray<niryo_one_msgs::SetLedsRequest::_values_type::const_iterator, int32_t>({1, msg->values.size()}, msg->values.begin(), msg->values.end());
