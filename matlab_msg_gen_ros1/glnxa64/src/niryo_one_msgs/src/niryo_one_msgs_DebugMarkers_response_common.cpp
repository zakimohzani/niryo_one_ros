...
// Common copy functions of server for niryo_one_msgs/DebugMarkersResponse
...
namespace DebugMarkers_Response {
  ...
  MDArray_T get_arr_sensor_msgs_CompressedImage(MDFactory_T& factory, const niryo_one_msgs::DebugMarkers::Response::_img_type& val);
  ...
  //----------------------------------------------------------------------------
  NIRYO_ONE_MSGS_EXPORT void copy_from_arr(niryo_one_msgs::DebugMarkers::Response& msg, const matlab::data::StructArray arr) {
    try {
        //markers_detected
        const matlab::data::TypedArray<bool> markers_detected_arr = arr[0]["markers_detected"];
        msg.markers_detected = markers_detected_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'markers_detected' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'markers_detected' is wrong type; expected a logical.");
    }
    try {
        //img
  ...
  //----------------------------------------------------------------------------
  NIRYO_ONE_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const niryo_one_msgs::DebugMarkers::Response& msg) {
    auto outArray = factory.createStructArray({1,1},{"markers_detected","img"});
    // markers_detected
    outArray[0]["markers_detected"] = factory.createScalar(static_cast<bool>(msg.markers_detected));
    // img
