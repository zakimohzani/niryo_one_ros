...
// Common copy functions of server for niryo_one_msgs/GetDigitalIOResponse
...
namespace GetDigitalIO_Response {
  //----------------------------------------------------------------------------
  NIRYO_ONE_MSGS_EXPORT void copy_from_arr(niryo_one_msgs::GetDigitalIO::Response& msg, const matlab::data::StructArray arr) {
    try {
        //status
    ...
    try {
        //pin
        const matlab::data::TypedArray<int32_t> pin_arr = arr[0]["pin"];
        msg.pin = pin_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'pin' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'pin' is wrong type; expected a int32.");
    }
    try {
        //name
    ...
    try {
        //state
        const matlab::data::TypedArray<int32_t> state_arr = arr[0]["state"];
  ...
  //----------------------------------------------------------------------------
  NIRYO_ONE_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const niryo_one_msgs::GetDigitalIO::Response& msg) {
    auto outArray = factory.createStructArray({1,1},{"status","message","pin","name","mode","state"});
    // status
