...
namespace generatetrajRequest {
  //----------------------------------------------------------------------------
  NIRYO_ONE_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<niryo_one_msgs::generatetrajRequest>& msg, const matlab::data::StructArray arr) {
    try {
        //j0
  ...
  //----------------------------------------------------------------------------
  NIRYO_ONE_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const niryo_one_msgs::generatetrajRequest>& msg) {
    auto outArray = factory.createStructArray({1,1},{"j0","j1"});
    // j0
    outArray[0]["j0"] = factory.createArray<niryo_one_msgs::generatetrajRequest::_j0_type::const_iterator, double>({1, msg->j0.size()}, msg->j0.begin(), msg->j0.end());
    // j1
    outArray[0]["j1"] = factory.createArray<niryo_one_msgs::generatetrajRequest::_j1_type::const_iterator, double>({1, msg->j1.size()}, msg->j1.begin(), msg->j1.end());
